...
// HELPERS
// ─────────────────────────────────────────────────────────────────────────────
const dec = (n: number) => n; // Prisma accepts plain JS numbers for Decimal fields
...
// ─────────────────────────────────────────────────────────────────────────────
async function seedVendorPlans(categories: Awaited<ReturnType<typeof seedCategories>>) {
...
// ─────────────────────────────────────────────────────────────────────────────
async function seedAttributeDefinitions(categories: Awaited<ReturnType<typeof seedCategories>>) {
...
async function seedProductAttributes(
  ...
  defs: Awaited<ReturnType<typeof seedAttributeDefinitions>>,
) {
...
async function seedVendorProfiles(
  vendors: { id: string }[],
  plans: Awaited<ReturnType<typeof seedVendorPlans>>,
) {
...
async function seedProducts(
  ...
  const iphone = await prisma.product.create({
    data: {
      ...
      status: ProductStatus.active, isFeatured: true,
      images: {
        create: [
  ...
  const headphones = await prisma.product.create({
    data: {
      ...
      status: ProductStatus.active, isFeatured: false,
      images: {
        create: [
...
// PHASE 1 — PROMOTIONS
// ─────────────────────────────────────────────────────────────────────────────
async function seedPromotions(techVendorId: string) {
...
// ─────────────────────────────────────────────────────────────────────────────
async function seedOrderWorkflows(
  admin: { id: string },
  vendors: { id: string }[],
  customers: { id: string }[],
  vendorProfiles: { id: string; commissionRate: any }[],
  products: Awaited<ReturnType<typeof seedProducts>>,
  promotions: Awaited<ReturnType<typeof seedPromotions>>,
) {
  ...
  await prisma.orderStatusHistory.createMany({
    data: [
      { orderId: orderA.id, previousStatus: OrderStatus.pending,    newStatus: OrderStatus.confirmed,   note: 'Order confirmed by vendor',          changedById: vendors[0].id, createdAt: daysAgo(15) },
      { orderId: orderA.id, previousStatus: OrderStatus.confirmed,  newStatus: OrderStatus.processing,  note: 'Items picked and packed',            changedById: vendors[0].id, createdAt: daysAgo(14) },
      { orderId: orderA.id, previousStatus: OrderStatus.processing, newStatus: OrderStatus.shipped,     note: 'Shipped via FedEx Express',          changedById: vendors[0].id, createdAt: daysAgo(13) },
      { orderId: orderA.id, previousStatus: OrderStatus.shipped,    newStatus: OrderStatus.delivered,   note: 'Delivered to customer successfully', changedById: admin.id,      createdAt: daysAgo(10) },
    ],
  });
    ...
    data: [
      {
        ...
        status: PaymentStatus.completed, paidAt: daysAgo(5),
  ...
  await prisma.orderStatusHistory.createMany({
    data: [
      { orderId: orderB.id, previousStatus: OrderStatus.pending,    newStatus: OrderStatus.confirmed,  note: 'Order confirmed',     changedById: vendors[0].id, createdAt: daysAgo(5) },
      { orderId: orderB.id, previousStatus: OrderStatus.confirmed,  newStatus: OrderStatus.processing, note: 'Packing items',       changedById: vendors[0].id, createdAt: daysAgo(4) },
      { orderId: orderB.id, previousStatus: OrderStatus.processing, newStatus: OrderStatus.shipped,    note: 'Handed to FedEx',     changedById: vendors[0].id, createdAt: daysAgo(3) },
    ],
  });
  ...
  await prisma.vendorCommission.createMany({
    data: [
      { vendorId: techVendor.id, orderId: orderB.id, orderItemId: itemB1.id, grossAmount: dec(commB1_gross), commissionRate: dec(8.00), commissionAmount: dec(commB1_comm), netAmount: dec(commB1_net), status: PaymentStatus.pending, createdAt: daysAgo(3) },
      { vendorId: techVendor.id, orderId: orderB.id, orderItemId: itemB2.id, grossAmount: dec(commB2_gross), commissionRate: dec(8.00), commissionAmount: dec(commB2_comm), netAmount: dec(commB2_net), status: PaymentStatus.pending, createdAt: daysAgo(3) },
    ],
  });
  ...
  await prisma.orderStatusHistory.createMany({
    data: [
      { orderId: orderC.id, previousStatus: OrderStatus.pending,   newStatus: OrderStatus.confirmed,  note: 'Payment received',    changedById: vendors[1].id, createdAt: daysAgo(2) },
      { orderId: orderC.id, previousStatus: OrderStatus.confirmed, newStatus: OrderStatus.processing, note: 'Preparing your items', changedById: vendors[1].id, createdAt: daysAgo(1) },
    ],
  });
  ...
  await prisma.vendorCommission.createMany({
    data: [
      { vendorId: fashionVendor.id, orderId: orderC.id, orderItemId: itemC1.id, grossAmount: dec(commC1_gross), commissionRate: dec(10.00), commissionAmount: dec(commC1_comm), netAmount: dec(commC1_net), status: PaymentStatus.pending, createdAt: daysAgo(1) },
      { vendorId: fashionVendor.id, orderId: orderC.id, orderItemId: itemC2.id, grossAmount: dec(commC2_gross), commissionRate: dec(10.00), commissionAmount: dec(commC2_comm), netAmount: dec(commC2_net), status: PaymentStatus.pending, createdAt: daysAgo(1) },
    ],
  });
  ...
  await prisma.orderStatusHistory.createMany({
    data: [
      { orderId: orderE.id, previousStatus: OrderStatus.pending,    newStatus: OrderStatus.confirmed,  note: 'Order confirmed',                           changedById: vendors[0].id, createdAt: daysAgo(20) },
      { orderId: orderE.id, previousStatus: OrderStatus.confirmed,  newStatus: OrderStatus.processing, note: 'Items prepared',                            changedById: vendors[0].id, createdAt: daysAgo(19) },
      { orderId: orderE.id, previousStatus: OrderStatus.processing, newStatus: OrderStatus.shipped,    note: 'Shipped via FedEx',                         changedById: vendors[0].id, createdAt: daysAgo(18) },
      { orderId: orderE.id, previousStatus: OrderStatus.shipped,    newStatus: OrderStatus.delivered,  note: 'Delivered',                                 changedById: admin.id,      createdAt: daysAgo(15) },
      { orderId: orderE.id, previousStatus: OrderStatus.delivered,  newStatus: OrderStatus.refunded,   note: 'Return approved — refund issued to customer', changedById: admin.id,    createdAt: daysAgo(3)  },
    ],
  });
  ...
  await prisma.notification.createMany({
    data: [
      ...
      { userId: c1.id, type: NotificationType.order,   title: { en: 'Order Placed!',     ar: 'تم تقديم الطلب!' },     body: { en: 'Your order ORD-20260501-0001 has been placed successfully.', ar: 'تم تقديم طلبك ORD-20260501-0001 بنجاح.' },       data: { orderId: orderA.id }, isRead: true,  createdAt: daysAgo(15) },
      { userId: c1.id, type: NotificationType.order,   title: { en: 'Order Confirmed',   ar: 'تم تأكيد الطلب' },      body: { en: 'TechZone Guam confirmed your order.',                        ar: 'تيك زون غوام أكدت طلبك.' },                         data: { orderId: orderA.id }, isRead: true,  createdAt: daysAgo(15) },
      { userId: c1.id, type: NotificationType.order,   title: { en: 'Order Shipped!',    ar: 'تم شحن طلبك!' },        body: { en: 'Your order is on the way. Tracking: FX-7741234560-GU',       ar: 'طلبك في الطريق. رقم التتبع: FX-7741234560-GU' },     data: { orderId: orderA.id }, isRead: true,  createdAt: daysAgo(13) },
      { userId: c1.id, type: NotificationType.order,   title: { en: 'Order Delivered!',  ar: 'تم توصيل طلبك!' },      body: { en: 'Your order has been delivered. Enjoy your products!',        ar: 'تم توصيل طلبك. استمتع بمنتجاتك!' },                 data: { orderId: orderA.id }, isRead: true,  createdAt: daysAgo(10) },
      { userId: c1.id, type: NotificationType.review,  title: { en: 'Review Approved',   ar: 'تمت الموافقة على مراجعتك' }, body: { en: 'Your review for iPhone 15 Pro has been approved.', ar: 'تمت الموافقة على مراجعتك لآيفون 15 برو.' }, data: { productId: products.iphone.id }, isRead: false, createdAt: daysAgo(7) },
      // Customer1 — Order C
      { userId: c1.id, type: NotificationType.order,   title: { en: 'Order Processing',  ar: 'طلبك قيد التجهيز' },    body: { en: 'Island Fashion House is preparing your order ORD-20260501-0003.', ar: 'بيت الأزياء الجزيري يجهز طلبك ORD-20260501-0003.' }, data: { orderId: orderC.id }, isRead: false, createdAt: daysAgo(1) },
      // Customer2 — Order B
      { userId: c2.id, type: NotificationType.order,   title: { en: 'Order Shipped!',    ar: 'تم شحن طلبك!' },        body: { en: 'Your Samsung S24 Ultra is on the way! Track: FX-9921234561-GU', ar: 'سامسونج S24 الترا في الطريق! تتبع: FX-9921234561-GU' }, data: { orderId: orderB.id }, isRead: false, createdAt: daysAgo(3) },
      // Customer3 — Order E refund
      { userId: c3.id, type: NotificationType.payment, title: { en: 'Refund Processed',  ar: 'تمت معالجة الاسترداد' }, body: { en: 'Your refund of $3,199.00 for order ORD-20260501-0005 has been processed.', ar: 'تمت معالجة استرداد 3,199.00 دولار لطلبك ORD-20260501-0005.' }, data: { orderId: orderE.id }, isRead: false, createdAt: daysAgo(3) },
      // Vendor1 (TechZone) — new review alert
      { userId: vendors[0].id, type: NotificationType.review, title: { en: 'New 5★ Review!', ar: 'مراجعة 5 نجوم جديدة!' }, body: { en: 'Mohammed Al-Zaidi left a 5-star review on iPhone 15 Pro.', ar: 'محمد الزيدي ترك مراجعة 5 نجوم على آيفون 15 برو.' }, data: { productId: products.iphone.id }, isRead: false, createdAt: daysAgo(8) },
      { userId: vendors[0].id, type: NotificationType.order,  title: { en: 'Return Request',  ar: 'طلب إرجاع' },          body: { en: 'Khalid Al-Amri requested a return for order ORD-20260501-0005.', ar: 'خالد العامري طلب إرجاع الطلب ORD-20260501-0005.' }, data: { orderId: orderE.id }, isRead: true, createdAt: daysAgo(5) },
      // Admin — system alerts
      { userId: admin.id,      type: NotificationType.system, title: { en: 'New Vendor Application', ar: 'طلب بائع جديد' }, body: { en: 'A new vendor has applied to join Hagatna platform.', ar: 'تقدم بائع جديد للانضمام إلى منصة هاجاتنا.' }, data: {}, isRead: true, createdAt: daysAgo(45) },
      { userId: admin.id,      type: NotificationType.order,  title: { en: 'Return Approved',  ar: 'تمت الموافقة على الإرجاع' }, body: { en: 'Return for ORD-20260501-0005 approved. Refund $3,199 issued.', ar: 'تمت الموافقة على إرجاع ORD-20260501-0005. صرف استرداد 3,199 دولار.' }, data: { orderId: orderE.id }, isRead: false, createdAt: daysAgo(3) },
    ],
  });
